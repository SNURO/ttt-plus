...
def obtain_shot_label(loader, ext, task_head, args, c=None):
    ...
    if args.perfect_label:
        return all_label
        
    all_output = nn.Softmax(dim=1)(all_output)
    _, predict = torch.max(all_output, 1)
    accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0]) # unknown in fact
    ...
    print(log_str+'\n')
    #import ipdb; ipdb.set_trace(cond=True)
